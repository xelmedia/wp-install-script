...
echo "$scriptContent" | wp-env run tests-cli tee /var/www/html/WPInstallScript.php > /dev/null
echo "$htaccessContent" | wp-env run tests-cli tee /var/www/html/.htaccess > /dev/null
...
cat .db.env | wp-env run tests-cli tee /var/www/html/.db.env > /dev/null

cleanupContent=$(<cleanup.sh)
echo "$cleanupContent" | wp-env run tests-cli tee /var/www/html/cleanup.sh > /dev/null
wp-env run tests-cli chmod +x cleanup.sh
wp-env run tests-cli ./cleanup.sh
wp-env run tests-cli php WPInstallScript.php -p p -d d
php test.php